-- The orders table already has is_scheduled/scheduled_date/scheduled_time/
-- scheduled_msg columns (added outside this session's tracked migrations),
-- and checkout.html already has a "تاريخ التوصيل المفضل" / "الوقت المفضل"
-- date+time picker in the form — but neither create_order nor
-- create_cart_order ever accepted those values, so every customer pick
-- was silently discarded and never reached the DB. The admin's own manual
-- "طلب مجدول" entry form was unaffected (it inserts those columns
-- directly), which is why that path "worked" while real customer
-- checkout never produced anything loadScheduled() could find.
--
-- ✅ Adding new params to an existing function signature with CREATE OR
-- REPLACE creates a SECOND overload instead of replacing the original
-- (bit us before in migration_35) — DROP the old signature first.

begin;

drop function if exists public.create_order(
  text, text, text, integer, text, text, text, text, integer, text, text, text, text, text, text, text, boolean, text, text
);
drop function if exists public.create_cart_order(
  text, jsonb, text, text, integer, text, text, text, text, text, text, text, text, text, boolean, text, text
);
...
begin
  ...
  insert into public.orders (
    ...
    is_scheduled, scheduled_date, scheduled_time, created_at
  ) values (
    ...
    (p_scheduled_date is not null), p_scheduled_date, nullif(p_scheduled_time,''), now()
  )
  returning
