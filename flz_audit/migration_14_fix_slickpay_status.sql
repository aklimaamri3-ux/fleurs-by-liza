-- Fix: orders_secure_insert put every non-COD, non-pickup order (including
-- cib/dahabia — SlickPay-routed card payments) into 'waiting_review', the
-- SAME status used by genuine bank-transfer orders awaiting a manually
-- uploaded receipt. slickpay_create immediately PATCHes cib/dahabia orders
-- to 'waiting_slickpay', but if that call fails, is slow, or the customer
-- abandons before it completes, the order sits at 'waiting_review' —
-- indistinguishable in the admin "Receipts" review queue from a real
-- bank-transfer order, with an "Approve" button that marks it paid with
-- zero verification. Card/gateway orders must never be reachable from
-- that manual-approval queue: only slickpay_check / the SlickPay webhook,
-- both of which re-verify against SlickPay's own API, may ever set
-- payment_status='paid' for these payment methods.
...
BEGIN
  ...
  ELSIF NEW.payment_method IN ('cib', 'dahabia') THEN
    -- ✅ SlickPay-routed card payment: only slickpay_check / the SlickPay
    -- webhook (both re-verify against SlickPay's own API) may ever move
    -- this to 'paid'. Must never land in the manual-receipt-review queue.
    NEW.payment_status := 'pending';
  ELSE
    NEW.payment_status := 'waiting_review';
  END IF;
